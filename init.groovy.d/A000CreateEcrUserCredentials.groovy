...
def ecr_user_credentials_file = "/var/jenkins_home/init.groovy.d/ecr_service_user_credentials.ini"
...
static Map<String, String> getCredentialsFromINIfile(String filename){
    def ini = new IniParser(filename)
    ini.dumpConfig()
    def secs = ini.getAllSections()
    def username = secs[0]
    def access_key_id = ini.getConfig(username, 'aws_access_key_id')
    def secret_access_key =  ini.getConfig(username, 'aws_secret_access_key')
    return [
        'username': username,
...
Map<String, String> access_keys = getCredentialsFromINIfile(ecr_user_credentials_file)
